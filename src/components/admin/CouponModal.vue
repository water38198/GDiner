    ...
    <VForm method="dialog" v-slot="{ errors }" @submit="$emit('confirmCoupon', tempCoupon)">
      ...
      <div class="grid grid-cols-2 gap-6 p-5">
        <div class="input-group">
          ...
          <VField type="text" id="name" name="名稱" rules="required" placeholder="請輸入名稱" v-model.trim="tempCoupon.title"
        ...
        <div>
          <label for="is_paid">
            <input type="checkbox" id="is_paid" v-model="tempCoupon.is_enabled" class="custom-checkbox" :true-value="1"
              :false-value="0">
            是否啟用
          </label>
        </div>
